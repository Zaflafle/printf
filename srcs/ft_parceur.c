...
/*   Updated: 2018/03/08 22:46:59 by macuguen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_choice_instruction(va_list *args, char *format, t_printf *list, int *ret)
{
	if (ft_structur_conv(list) == 1 || ft_structur_conv(list) == 2)
	...
	else if (ft_structur_conv(list) == 7)
		...
		*ret = *ret + ft_instruction_unis(args, (char*)format, list);
...
int				ft_parceur(t_printf *list, char *format, va_list *args)
{
	...
	while (format[i] != '\0')
	{
		...
		(format[i] == '%' && format[i + 2] != '\0' && format[i + 2] != '\n'))
		if (format[i] == '%')
